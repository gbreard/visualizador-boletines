...
def load_all_data():
    """
    Carga todos los CSV y la tabla de descriptores una sola vez.
    ...
    for file_name in files_to_load:
        ...
        if os.path.exists(file_path):
            ...
            if file_name in ['C1.1', 'C1.2', 'C2.1', 'C2.2', 'C3', 'C4', 'C5', 'C6', 'C7']:
                df = calculate_variations(df)
            
            data[file_name] = df
            logger.info(f"  {file_name}: {len(df)} registros cargados")
